...
public abstract class SoundEffectBreathWeapon {
    ...
    private void setAllStopFlags() {
        if (headStartupSound!=null) {
            headStartupSound.setDonePlaying();
        }
        if (headLoopSound!=null) {
            headLoopSound.setDonePlaying();
        }
        if (headStoppingSound!=null) {
            headStoppingSound.setDonePlaying();
        }
    }
    ...
    public void performTick(EntityPlayerSP entityPlayerSP) {
        ...
        headSoundSettings.playerDistanceToEpicentre=(float) weaponSoundInfo.dragonHeadLocation.distanceTo(entityPlayerSP.getPositionVector());
        ...
        if (weaponSoundInfo.breathingState!=currentWeaponState) {
            switch (weaponSoundInfo.breathingState) {
                case IDLE: {
                    ...
                    headStoppingSound=ComponentSound.createComponentSound(headStop, HEAD_MIN_VOLUME, ComponentSound.RepeatType.NO_REPEAT, headSoundSettings);
                ...
                case BREATHING: {
                    ...
                    headStartupSound=ComponentSound.createComponentSound(headStart, HEAD_MIN_VOLUME, ComponentSound.RepeatType.NO_REPEAT, headSoundSettings);
                    headStartupSound.setPlayCountdown(headStart.getDurationInTicks());
                    soundController.playSound(headStartupSound);
                    break;
                }
                default: {
                    System.err.printf("Illegal weaponSoundInfo.breathingState:" + weaponSoundInfo.breathingState + " in " + this.getClass());
                }
            }
        ...
        switch (currentWeaponState) {
            case BREATHING: {
                if (headStartupSound!=null && headStartupSound.getPlayCountdown() <= 0) {
                    stopAllHeadSounds();
                    SoundEffectNames headLoop=weaponHeadSound(SoundPart.LOOP, weaponSoundInfo.lifeStage);
                    headLoopSound=ComponentSound.createComponentSound(headLoop, HEAD_MIN_VOLUME, ComponentSound.RepeatType.REPEAT, headSoundSettings);
     ...
     */
    abstract protected SoundEffectNames weaponHeadSound(SoundPart soundPart, EnumDragonLifeStage lifeStage);
