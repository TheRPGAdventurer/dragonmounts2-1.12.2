...
public class DragonBreedNether extends DragonBreed {
    ...
    public void onLivingUpdate(EntityTameableDragon dragon) {
        ...
        if (world instanceof WorldServer && dragon.isWet() && !dragon.isEgg()) {
            ((WorldServer) world).spawnParticle(EnumParticleTypes.SMOKE_NORMAL, dragon.posX + 0.5D, dragon.posY + dragon.getEyeHeight(), dragon.posZ + 0.5D, 4 * (int) dragon.getScale(), 0.5D, 0.25D, 0.5D, 0.0D);
        }

        if (world instanceof WorldServer && !dragon.isDead && !dragon.isEgg()) {
            ((WorldServer) world).spawnParticle(EnumParticleTypes.DRIP_LAVA, dragon.posX, dragon.posY + dragon.getEyeHeight(), dragon.posZ, 1, 0.5D, 0.25D, 0.5D, 0.0D);
        }
    }
