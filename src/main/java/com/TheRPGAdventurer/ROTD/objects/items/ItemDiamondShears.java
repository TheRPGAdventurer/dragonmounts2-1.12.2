...
public class ItemDiamondShears extends ItemShears implements IHasModel {
    ...
    public boolean itemInteractionForEntity(ItemStack itemstack, EntityPlayer player, EntityLivingBase entity, net.minecraft.util.EnumHand hand) {
        ...
        if (entity instanceof EntityTameableDragon) {
            ...
            if (target.isShearable(itemstack, target.world, pos)) {
                List<ItemStack> drops=target.onSheared(itemstack, target.world, pos, EnchantmentHelper.getEnchantmentLevel(net.minecraft.init.Enchantments.FORTUNE, itemstack));
    ...
    public EntityItem dropItem(ItemStack stack, EntityItem entityIn, EntityPlayer player) {
        if (!player.world.isRemote) {
            player.getEntityWorld().spawnEntity(entityIn);
        }
