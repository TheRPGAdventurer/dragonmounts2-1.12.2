...
public class ItemDragonArmour extends ItemArmor {
	...
	public void onArmorTick(World world, EntityPlayer player, ItemStack itemStack) {
		...
		if(chest == ModArmour.forestDragonScaleTunic && player.getAttackingEntity()!= null && player.attackEntityFrom(DamageSource.causeMobDamage(player.getAttackingEntity()), (float)player.getAttackingEntity().getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue()) && new Random().nextInt(25) == 1) player.addPotionEffect(new PotionEffect(MobEffects.REGENERATION, 200,0, false, false));
		...
		&& (player.isInLava() || player.world.isMaterialInBB(player.getEntityBoundingBox().grow(-0.1, -0.4, -0.1), Material.FIRE)))
